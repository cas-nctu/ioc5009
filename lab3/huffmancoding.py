...
def huffman_encode(arr, prefix, save_dir='./'):
    ...
    datasize = dump(data_encoding, directory/'{}.bin'.format(prefix))

    # Dump codebook (huffman tree)
    codebook_encoding = encode_huffman_tree(root, dtype)
    treesize = dump(codebook_encoding, directory/'{}_codebook.bin'.format(prefix))
...
def huffman_decode(directory, prefix, dtype):
    ...
    # Read the codebook
    codebook_encoding = load(directory/'{}_codebook.bin'.format(prefix))
    root = decode_huffman_tree(codebook_encoding, dtype)

    # Read the data
    data_encoding = load(directory/'{}.bin'.format(prefix))
...
def load(filename):
    ...
    with open(filename, 'rb') as f:
        header = f.read(1)
        rest = f.read() # bytes
        code_str = ''.join('{:08b}'.format(byte) for byte in rest)
        offset = ord(header)
        if offset != 0:
...
def huffman_encode_model(model, directory='encodings/'):
    ...
    original_total = 0
    compressed_total = 0
    print("{:<15} | {:>10} {:>10} {:>11} {:>7}".format('Layer','original','compressed','improvement','percent'))
    print('-'*70)
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            # Encode
            t0, d0 = huffman_encode(mat.data, name+'_{}_data'.format(form), directory)
            t1, d1 = huffman_encode(mat.indices, name+'_{}_indices'.format(form), directory)
            t2, d2 = huffman_encode(calc_index_diff(mat.indptr), name+'_{}_indptr'.format(form), directory)

            # Print statistics
            original = mat.data.nbytes + mat.indices.nbytes + mat.indptr.nbytes
            compressed = t0 + t1 + t2 + d0 + d1 + d2

            print("{:<15} | {:10} {:10} {:>10.2f}x {:>6.2f}%".format(name,original,compressed,original / compressed,100 * compressed / original))
        else: # bias
            # Note that we do not huffman encode bias
            bias = param.data.cpu().numpy()
            bias.dump('%s/%s' % (directory, name))
    ...
    print('-'*70)
    print("{:15} | {:>10} {:>10} {:>10.2f}x {:>6.2f}%".format('total',original_total,compressed_total,original_total / compressed_total,100 * compressed_total / original_total))
...
def huffman_decode_model(model, directory='encodings/'):
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            # Decode data
            data = huffman_decode(directory, name+'_{}_data'.format(form), dtype='float32')
            indices = huffman_decode(directory, name+'_{}_indices'.format(form), dtype='int32')
            indptr = reconstruct_indptr(huffman_decode(directory, name+'_{}_indptr'.format(form), dtype='int32'))
